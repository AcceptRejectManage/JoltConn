...
public class AppGdx extends ApplicationAdapter {
    ...
    public void create() {
        try {
            ...
            gdxSync = new GdxNetSync(sha);


            
            LocalStorage localStorage = new LocalStorage(new Configuration("./gameConfig.json", "gameA"), gdxSync);
            localStorage.writeMetaFilesToDisk();
            JoltStorage joltStorage = new JoltStorage(new Configuration("./gameConfig.json", "gameB"), gdxSync);
